...
When analysing a dataset often some summary statistics are needed of certain
columns. For example, the maximum and minimum value in a column, a frequency
table of all the values in a column, or the number of missing values in a
column. These can of course be calculated using a suitable function with
\texttt{process\_blocks}. However, as some of these will be used regularly some
functions have been implemented in the package. 
...
calculated. For example, to calculate the average age the following options are
available:
<<echo=TRUE>>=
m1 <- colmean(laf, columns=3)
m1 <- colmean(laf$age)
@
