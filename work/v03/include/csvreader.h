...
class CSVReader {
  public:
    ...
    CSVReader(const std::string& filename) : ncolumns_(0), ignore_columns_(false), 
      read_buffer_(filename), state_(NEWREC), quote_(BOTH) {
    }
  ...
  protected:
    // Callback functions: are called when e.g. end of line is reached
