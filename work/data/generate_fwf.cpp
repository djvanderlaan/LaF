...
int main(int argc, char** argv) {
  ...
  for(int i = 0; i < nsim; ++i) {
    ...
    // age
    std::cout << std::setw(2) << uniform_int(gen);
    ...
    // normal
    for (unsigned int j = 0; j < ncol; ++j) 
      std::cout << std::fixed << std::setw(6) << std::setprecision(2) << normal(gen);

    // log-normal
    for (unsigned int j = 0; j < ncol; ++j) 
      std::cout << std::fixed << std::setw(6) << std::setprecision(2) << lognormal(gen);
